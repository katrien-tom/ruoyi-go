...
func (h *Handler) List(c *gin.Context) {
	...
	if err != nil {
		response.Fail(c, response.InternalError, "failed to get login log list")
		return
	}
...
func (h *Handler) Delete(c *gin.Context) {
	ids := strings.Split(c.Param("infoIds"), ",")
	logIDs := make([]int64, 0, len(ids))
	for _, s := range ids {
		id, err := strconv.ParseInt(strings.TrimSpace(s), 10, 64)
		if err != nil {
			response.Fail(c, response.ParamError, "invalid info id: "+s)
			return
		}
	...
	if err := h.service.DeleteLogs(c.Request.Context(), logIDs); err != nil {
		response.Fail(c, response.InternalError, "failed to delete login logs")
		return
	}
	response.Success[any](c, nil, "delete login logs success")
}

func (h *Handler) Clean(c *gin.Context) {
	if err := h.service.Clean(c.Request.Context()); err != nil {
		response.Fail(c, response.InternalError, "failed to clean login logs")
		return
	}
	response.Success[any](c, nil, "clean login logs success")
}
